...
function LiveMissionCountdown({
  ...
  return (
    <div
      className={`p-2 px-3 rounded-2xl border transition-all duration-300 flex items-center justify-between gap-3 shadow-xs ${isDone
    ...
    >
      ...
      <div className="flex items-center gap-2.5 min-w-0 flex-1">
        <button
          ...
          className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all cursor-pointer ${isDone
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span
              className={`text-xs font-bold truncate block ${isDone ? 'line-through text-slate-400' : 'text-slate-900'
                }`}
            >
              {task.title}
            </span>
      ...
      <div className="flex items-center gap-2 shrink-0">
        {isDone ? (
          ...
          <div
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-rose-400/80 font-mono font-black tracking-wider text-xs shadow-md shadow-rose-600/25 bg-gradient-to-r from-[#e11d48] to-[#be123c] text-white transition-all ${isUrgent ? 'animate-pulse ring-2 ring-rose-400' : ''
              }`}
          >
...
export const WeeklyConsistencyOverview: React.FC<WeeklyConsistencyOverviewProps> = ({
  ...
  return (
    <section className={`w-full rounded-2xl sm:rounded-3xl p-3 sm:p-5 lg:p-6 border transition-all duration-300 ${isDarkMode
      ...
      <div className="dashboard-weekly-grid grid grid-cols-1 lg:grid-cols-12 gap-3.5 sm:gap-5 lg:gap-6 items-stretch">
          ...
          <div className="w-full flex-1 min-h-[220px] rounded-2xl overflow-hidden relative shadow-inner flex items-center justify-center bg-slate-950">
            ...
            <div className="absolute top-2.5 right-2.5 z-30 flex items-center gap-1.5">
              <button
                ...
                className={`p-1.5 rounded-full backdrop-blur-md transition-all duration-200 border shadow-md cursor-pointer flex items-center gap-1 text-[9px] sm:text-[10px] font-bold px-2.5 ${isMantraReelActive
        ...
        <div className={`dashboard-weekly-chart lg:col-span-6 flex flex-col justify-between space-y-3 p-3.5 sm:p-5 rounded-2xl border transition-all duration-300 relative overflow-hidden min-h-[340px] sm:min-h-[380px] ${isDarkMode ? 'bg-[#121826]/90 border-slate-800/90 text-white shadow-md' : 'bg-white border-slate-200/90 text-slate-900 shadow-sm'
          }`}>

          {/* Real-Time Emergency Mission Countdown Banner */}
          {activeEmergencyTasks.length > 0 && (
            <div className={`p-2.5 sm:p-3 rounded-2xl border space-y-2 shadow-2xs ${isDarkMode
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <span className={`text-[10px] sm:text-[11px] font-black uppercase tracking-wider font-mono flex items-center gap-1 ${isDarkMode ? 'text-rose-300' : 'text-rose-700'
                    }`}>
                    Emergency Sprint
                  </span>
          ...
          {/* Week Header Labels */}
          <div className={`grid grid-cols-5 text-center text-[10px] sm:text-xs font-serif-title italic font-black border-b pb-1.5 sm:pb-2 ${isDarkMode ? 'border-slate-800' : 'border-slate-200'
            }`}>
          ...
          <div className="overflow-x-auto touch-pan-x pb-1 relative scroller-smooth">
            <div className="min-w-[320px] sm:min-w-[460px] h-28 sm:h-32 flex items-end justify-between gap-0.5 sm:gap-1 pt-1">
              {daysData.map((d) => (
                ...
                >
                  <div className={`w-full rounded-t-md h-20 sm:h-24 flex items-end overflow-hidden p-0.5 transition-colors ${isDarkMode ? 'bg-slate-900/80 group-hover:bg-slate-800/80' : 'bg-slate-100 group-hover:bg-slate-200'
                    }`}>
                    <div
            ...
            <div className={`min-w-[320px] sm:min-w-[460px] flex items-center justify-between text-[7px] sm:text-[8px] font-mono font-bold pt-1 border-t ${isDarkMode ? 'text-slate-400 border-slate-800' : 'text-slate-600 border-slate-200'
              }`}>
              {daysData.map((d) => (
            ...
            <div className={`min-w-[320px] sm:min-w-[460px] flex items-center justify-between text-[8px] sm:text-[9px] font-mono font-black pt-0.5 ${isDarkMode ? 'text-white' : 'text-black'
              }`}>
              {daysData.map((d) => (
          ...
          <div className={`grid grid-cols-5 gap-1 sm:gap-2 pt-2 sm:pt-2.5 border-t ${isDarkMode ? 'border-slate-800' : 'border-slate-200'
            }`}>
            {weeksSummary.map((w) => {
              const radius = 18;
        ...
        <div className={`dashboard-leaderboard lg:col-span-3 flex flex-col justify-between rounded-2xl border p-3.5 sm:p-4.5 shadow-sm min-h-[340px] sm:min-h-[380px] transition-all duration-300 ${isDarkMode ? 'bg-[#121826]/90 border-slate-800/90 text-white' : 'bg-white border-slate-200/90 text-slate-900'
          }`}>
          {/* Header */}
          <div className={`flex items-center justify-between pb-2 sm:pb-2.5 border-b ${isDarkMode ? 'border-slate-800' : 'border-slate-200'
            }`}>
            ...
            <span className={`text-[8px] sm:text-[9px] font-black px-2 py-0.5 rounded-full font-mono ${isDarkMode ? 'bg-amber-950/70 text-amber-300 border border-amber-800' : 'bg-amber-100 text-amber-900 border border-amber-300/50'
              }`}>
              Ranked
            </span>
          </div>
          ...
          {/* Table Header */}
          <div className={`flex items-center justify-between text-[8px] sm:text-[9px] font-serif-title italic font-black px-2.5 py-1 sm:py-1.5 rounded-lg mt-2 sm:mt-2.5 border ${isDarkMode ? 'bg-slate-900/80 border-slate-800 text-slate-300' : 'bg-slate-100 border-slate-200 text-black'
            }`}>
          ...
          <div className="space-y-1 sm:space-y-1.5 my-2 sm:my-2.5 flex-1 overflow-y-auto max-h-[160px] sm:max-h-[175px] pr-1">
            {topHabits.slice(0, 10).map((h) => {
              ...
              return (
                <div key={h.name} className={`flex items-center justify-between text-xs py-1 px-1 rounded transition-colors ${isDarkMode ? 'hover:bg-slate-900/60' : 'hover:bg-slate-50'
                  }`}>
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                    <div className={`w-10 sm:w-12 rounded-full h-1.5 overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'
                      }`}>
                      <div
          ...
          {/* Subtitle Footer */}
          <div className={`pt-2 sm:pt-2.5 border-t text-[9px] sm:text-[10px] text-center font-bold italic ${isDarkMode ? 'border-slate-800 text-slate-400' : 'border-slate-200 text-slate-700'
            }`}>
